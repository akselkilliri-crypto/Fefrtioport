...
// ====== ИНВЕРСИЯ ОСИ (если лево/право перепутаны) ======
const bool INVERT_X = false;  // поставьте true, чтобы поменять лево и право местами
...
void loop() {
  if (bleGamepad.isConnected()) {
    ...
    if (raw < ZONE_CENTER_LOW) {
      // ЛЕВО: чем меньше raw, тем сильнее поворот
      steering = map(raw, ZONE_CENTER_LOW, 0, 0, 32767);
      steering = constrain(steering, 0, 32767);
      if (!INVERT_X) steering = -steering;  // обычно лево – отрицательное
    }
    else if (raw > ZONE_CENTER_HIGH) {
      // ПРАВО: чем больше raw, тем сильнее поворот
      steering = map(raw, ZONE_CENTER_HIGH, 4095, 0, 32767);
      steering = constrain(steering, 0, 32767);
      if (INVERT_X) steering = -steering;   // инвертируем, если нужно
    }
    else {
      // ЦЕНТР
      steering = 0;
